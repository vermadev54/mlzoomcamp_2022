...
model_ref = bentoml.sklearn.get("spam_classification:7suybouyqkbdblg6")
dv = model_ref.custom_objects['CountVectorizer']
model_runner = model_ref.to_runner()

svc = bentoml.Service("Spam_classify", runners=[model_runner])

@svc.api(input=JSON(pydantic_model=Spamtext), output=JSON())
async def classify(application_data):
    application_data=application_data.dict()
    print("application_data",application_data)
    application_data["message"] = text_preprocessing(application_data["message"])
    print(application_data["message"])

    vector = dv.transform([application_data["message"]])
    print(vector)
